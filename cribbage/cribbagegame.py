...
class CribbageGame:
    ...
    def __init__(self, players):
        ...
        self.board = CribbageBoard(
            self.players, self.MAX_SCORE
        )  #: the cribbage board for scoring
...
class CribbageRound:
    ...
    def _populate_crib(self):
        ...
        for p in self.game.players:
            ...
            if not set(cards_to_crib).issubset(set(self.hands[p])):
                raise IllegalCardChoiceError(
                    "Crib cards selected are not part of player's hand."
                )
        ...
        assert len(self.crib) == self.game.CRIB_SIZE, (
            "Crib size is not %s" % self.game.CRIB_SIZE
        )
    ...
    def get_table_value(self, sequence_start_idx):
        ...
        """
        return (
            sum(i["card"].get_value() for i in self.table[sequence_start_idx:])
            if self.table
            else 0
        )
    ...
    def play(self):
        ...
        while sum([len(v) for v in self.hands.values()]):
            ...
            while active_players:
                for p in active_players:
                    ...
                    card = p.select_card_to_play(
                        hand=self.hands[p],
                        table=self.table[sequence_start_idx:],
                        crib=self.crib,
                    )
                    if (
                        card.get_value() + self.get_table_value(sequence_start_idx) > 31
                        or card is None
                    ):
                        ...
                        if not self.hands[p]:
                            active_players.remove(p)
                        print(
                            "Player %s plays %s for %d"
                            % (
                                str(p),
                                str(card),
                                self.get_table_value(sequence_start_idx),
                            )
                        )
                        # Consider cards played by both players when scoring during play
                        assert (
                            self.get_table_value(sequence_start_idx) <= 31
                        ), "Value of cards on table must be <= 31 to be eligible for scoring."
                        score = self._score_play(
                            card_seq=[
                                move["card"] for move in self.table[sequence_start_idx:]
                            ]
                        )
                        if score:
            ...
            if len(active_players) == 0:
                ...
                active_players = [
                    p
                    for p in self.game.players
                    if p != player_of_last_card and self.hands[p]
                ]
        ...
        for p in self.game.players:
            p_cards_played = [
                move["card"] for move in self.table if move["player"] == p
            ]
            print(
                "Scoring " + str(p) + "'s hand: " + str(p_cards_played + [self.starter])
            )
            score = self._score_hand(
                cards=p_cards_played + [self.starter]
            )  # Include starter card as part of hand
            if score:
                self.game.board.peg(p, score)

        # Score the crib
        print("Scoring the crib: " + str(self.crib + [self.starter]))
        score = self._score_crib(
            cards=(self.crib + [self.starter])
        )  # Include starter card as part of crib
        if score:
            self.game.board.peg(self.dealer, score)

    @staticmethod
    def _score_play(card_seq):
        ...
        score_scenarios = [
            ...
            scoring.HasPairTripleQuad_DuringPlay(),
